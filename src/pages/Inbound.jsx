...
export default function Inbound({ titleModal }) {
    const [paylad, setPayload] = useState({
    date: null,
    stuff_id: null,
    total: null,
    proff_file: null,
...
function handleSubmitForm(e) {
    e.preventDefault();
    console.log(payload)
    axios.post(`http://localhost:8000/inbound-stuff/data`, payload, {
        headers: {
            'Authorization': 'Bearer ' + localStorage.getItem('token'),
        ...
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
                <h2 class="mb-4 text-xl font-bold text-white dark:text-white">Add a new Inbound Stuff Data</h2>
                {
                    alert ? (
                        ...
                        role="alert">
                        <span class="font-medium">Success!</span> check inbound data in <b><Link to="/inbound-stuff/data">this pages</Link></b>.
                        </div>
                    ) : ''
                }
                {
                ...
                <form onSubmit={handleSubmitForm}>
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <div class="grid gap-4 sm:grid-cols2 sm:gap-6">
                            </div>
                    <button type="" class="inline-flex items-center px-5 py-2.5 mt-4 sm:mt-6 text-sm font-medium text-center text-white bg-primary-700 rounded-lg focus:ring-4 focus:ring-primary-200 dark:focus:ring-primary-900 hover:bg-primary-800" >
                        Add Inbound
                    </button>
                    </form>
            </div>
